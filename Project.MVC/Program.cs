...
class Program
{
    class CustomerDemo
    {
        ...
        public List<OrderDemo> Orders { get; set; } // Bir customer'ýn birden fazla order' olabilir gösterimi aslýnda . (one to many)
    }
    ...
    class ProductDemo
    {
        ...
        public string Name { get; set; }
        public List<OrderDemo> orderDemos { get; set; }
    ...
    static void Main(string[] Args)
    {
        ...
        using (var db = new NorthwindContext())
        {
            //Not: Include() kýsmýna , Where(i=>i.Oders.Count()>0) yazmýþtýk , Burada Count() yerine Any() de kullanýlabilirdi .
            //Any() --> en az bir kaydýn olup olmadýðýný kontrol eder  , "count>0" da ayný iþlev de kullanýlýyor.

            var customers = db.Customers
                .Where(i => i.Orders.Any()) //Include(x => x.Orders) --> inner join yapan method . burada kullanýlabilirdi. 
                .Select(a => new CustomerDemo //Customer tablosundaki bazý alanlarý verileri , Demodaki oluþturduðum tabloya aktarýyorum.
                {
                    CustomerId = a.CustomerId,
                    Name = a.CompanyName,
                    OrderCount = a.Orders.Count(),
                    Orders = a.Orders.Select(i => new OrderDemo
                    {
                        OrderId = i.OrderId,
                        Total = (decimal)i.OrderDetails.Sum(od => od.Quantity * od.UnitPrice), // Her müþterinin sipariþinde ödediði toplam miktarý buluyoruz.
                        Products = i.OrderDetails.Select(p => new ProductDemo
                        {
            ...
            foreach (var customer in customers)
            {
                Console.WriteLine($" Adý : {customer.Name}   Sipariþ Sayýsý : {customer.OrderCount}");

                foreach (var orders in customer.Orders)
                {
                    Console.WriteLine($"Order Id : {orders.OrderId} Total : {orders.Total}");
        ...
        using (var db = new NorthwindContext()) 
        {
            //Ef Core ile sql komutlarý kullanýlarak db  ile baðlantý kurmak FromSqlRaw() methodu ile yapýlýr .
            var customers = db.Customers
                .FromSqlRaw("select * from Customers where city ={0}").ToList();
        }
       ...
       List<int> lastYear = new() { 55, 57, 53, 52, 56, 58, 59, 61, 62, 60, 58, 59 };
       List<int> currentYear = new() { 53, 56, 57, 54, 54, 58, 59, 60, 61, 63, 65, 61 };

        //Not:Bu seniki kilomuza göre , geçen seneki kilomuzun karþýlaþtýrýlmasý.

        var zipList = lastYear.Zip(currentYear, (last, current) => new { Last = last, Current = current }).ToList(); // 2 tane listeyi ortak bir tabloya toplar.

        var counter = 0;

        foreach (var item in zipList)
        {
            if (item.Current < item.Last)
            {
                var month = ((Months)counter).ToString(); // Ay ay enum ile counterý karþýlaþtýrýyor 
                Console.WriteLine($"{month} ayýnda Kilo verdim");
            }
              counter++;
        }
